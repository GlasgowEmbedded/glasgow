        ...
        if args.action == "run":
            if args.applet:
                ...
                if args.trace:
                    ...
                    vcd_writer = VCDWriter(args.trace, timescale='1 ns',
                ...
                async def run_analyzer():
                    ...
                    signals = {}
                    for name, width in trace_decoder.events():
                        *var_scope, var_name = "", name
                        signals[name] = vcd_writer.register_var(
                            scope=var_scope, name=var_name, var_type="wire", size=width,
                            init="x")
                    ...
                    while not trace_decoder.is_done():
                        ...
                        for cycle, events in trace_decoder.flush():
                            ...
                            timestamp  = 1e9 * cycle // target.sys_clk_freq
                            if init:
                                init = False
