...
class NBDServer:
    ...
    async def _fixed_newstyle_negotiation(self):
        ...
        self.logger.trace(f'Client flags: {client_flags}')
    ...
    async def _option_haggling(self):
        ...
        while not done:
            ...
            self.logger.trace(f"Option: {option}, {data=}")
    ...
    async def _transmission(self):
        ...
        while True:
            ...
            self.logger.trace(f"Command {req.command}, length {req.length}")
            if req.command == NBD_CMD_READ:
                ...
                data = await self.ep.recv(req.length)
