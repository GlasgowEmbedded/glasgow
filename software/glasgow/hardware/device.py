...
class GlasgowDevice:
        ...
        if self._mgmt_task is None:
            ...
            async def ep1_in_poller():
                while True:
                    ...
                    if serial == 0: # unsolicited reply
                        logger.trace("USB: BULK EP1 IN data=<%s> (unsolicited)", dump_hex(packet))
                        try:
                            await self._handle_unsolicited(payload)
                        except Exception as e:
                            logger.error("exception in unsolicited response handler", exc_info=True)
                    elif handler := self._mgmt_queue.pop(serial, None):
                        logger.trace("USB: BULK EP1 IN data=<%s> (result)", dump_hex(packet))
    ...
    @property
    def _max_current_lsbs(self):
        # Current measured by INA233 saturates at this value.
        return 32767
    ...
    async def download_bitstream(self, bitstream: bytes, bitstream_id: bytes = b"\xff" * 8,
        ...
        if not skip_health_check:
            # Each Glasgow bitstream has an I2C register at address 0, which is used to check that
            # the FPGA has configured properly and that the I2C bus function is intact. A small
            # subset of production devices manufactured by 1bitSquared fails this check.
            # Also, bitstreams which do not contain the Glasgow infrastructure will usually lack
            # an I2C target, and we want them to load successfully.
    ...
    def _index_to_iospec(self, index: int) -> str:
        return "ABCD"[index]
    ...
    async def set_trip_current(self, spec: str, amps: float):
        ...
        value = min(value, self._max_current_lsbs) # saturate
        result, = await self._command_fmt("<BB4H", "<B",
    ...
    async def _handle_unsolicited(self, result: bytes | bytearray):
        ...
        for index, port_alerts in enumerate([pa, pb, pc, pd]):
            ...
            for flag in GlasgowPortAlerts(port_alerts):
                match flag:
                    case GlasgowPortAlerts.UNDERVOLTAGE:
                        faults.append("sense undervoltage")
                    case GlasgowPortAlerts.OVERVOLTAGE:
                        faults.append("sense overvoltage")
                    case GlasgowPortAlerts.OVERCURRENT:
                        faults.append("supply overcurrent")
                    case _:
