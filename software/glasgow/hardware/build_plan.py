...
class GlasgowBuildPlan:
    ...
    async def execute_shell(self, build_dir: Optional[os.PathLike] = None, *,
        ...
        try:
            ...
            if await process.wait():
                _report_build_failure(stdout_lines, proc.returncode)
    ...
    async def _execute_js(self) -> tuple[bytes, bytes]:
        ...
        source_files = pyodide.ffi.to_js(self._inner.files)
    ...
    async def get_bitstream(self, *, debug=False) -> bytes:
        if sys.platform == "emscripten":
            # /cache is mounted as IDBFS, persisting it between page reloads
            cache_path = pathlib.Path("/cache")
        else:
            # locate the caches in the platform-appropriate cache directory; bitstreams aren't
            # large, but it is good etiquette to indicate to the OS that they can be wiped without
            # concern
