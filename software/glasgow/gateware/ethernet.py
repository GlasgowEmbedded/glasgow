...
from glasgow.gateware.stream import AsyncQueue, PacketQueue, PacketPadder
...
class LoopbackDriver(AbstractDriver):
    def elaborate(self, platform):
        ...
        m.d.comb += [
            self.cd_tx.clk.eq(ClockSignal()),
            self.cd_tx.rst.eq(ResetSignal()),
            self.cd_rx.clk.eq(ClockSignal()),
            self.cd_rx.rst.eq(ResetSignal()),
        ]
...
class Controller(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        m.domains.tx = self.driver.cd_tx
        m.domains.rx = self.driver.cd_rx

        m.submodules.tx_cdc_fifo = tx_cdc_fifo = AsyncQueue(
            shape=self.i.p.shape(), depth=8, o_domain="tx")
        wiring.connect(m, tx_cdc_fifo.i, wiring.flipped(self.i))

        m.submodules.tx_queue = tx_queue = DomainRenamer("tx")(
        ...
        m.submodules.tx_padder = tx_padder = DomainRenamer("tx")(
            PacketPadder(8, min_length=60, padding=0x00))
        with m.If(~self.tx_bypass):
            wiring.connect(m, tx_padder.i, tx_queue.o)

        m.submodules.fcs_appender = fcs_appender = DomainRenamer("tx")(
            ChecksumAppender(CRC32_ETHERNET))
        wiring.connect(m, fcs_appender.i, tx_padder.o)

        m.submodules.enframer = enframer = DomainRenamer("tx")(Enframer())
        with m.If(~self.tx_bypass):
        ...
        m.submodules.deframer = deframer = DomainRenamer("rx")(Deframer())
        wiring.connect(m, deframer.i, driver.o)

        m.submodules.fcs_verifier = fcs_verifier = DomainRenamer("rx")(
            ChecksumVerifier(CRC32_ETHERNET))
        with m.If(~self.rx_bypass):
            wiring.connect(m, fcs_verifier.i, deframer.o)

        m.submodules.rx_queue = rx_queue = DomainRenamer("rx")(
            PacketQueue(8, data_depth=2048, size_depth=32))
        with m.If(~self.rx_bypass):
            m.d.comb += [
        ...
        m.submodules.rx_cdc_fifo = rx_cdc_fifo = AsyncQueue(
            shape=rx_queue.o.p.shape(), depth=8, i_domain="rx")
