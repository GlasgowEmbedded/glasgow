...
from amaranth.lib import io, wiring, stream, cdc, enum, data
...
class JTAGProbeComponent(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.driver     = driver     = JTAGProbeDriver()
...
class JTAGProbeInterface(BaseJTAGProbeInterface):
    ...
    async def get_tck_freq(self) -> int:
        divisor = await self._divisor
        return round(1 / (2 * (divisor + 1) * self._sys_clk_period))

    async def set_tck_freq(self, frequency: int):
...
class JTAGProbeApplet(GlasgowAppletV2):
    ...
    async def setup(self, args):
        ...
        await self.jtag_iface.set_tck_freq(args.frequency * 1000)
