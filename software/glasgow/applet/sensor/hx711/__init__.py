...
class SensorHX711Controller(wiring.Component):
    ...
    osc frequency is freq(sync) / (2*(sck_divisor+1))
    """

    control_stream: In(stream.Signature(HX711Setting))
    """A transfer instructs :py:`SensorHX711Controller` to perform a bus transaction.

    The :py:`payload` is an :py:`Enum` representing one of the three valid combinations of channel
    and gain.
    ...
    complete and the acquired sample is transferred out on :py:`sample_stream`
    """

    sample_stream: Out(stream.Signature(Shape(24, signed=True)))
    """Stream for samples read from the HX711

    The :py:`payload` is a signed 24 bit integer representing the raw value of the sample.
    """

    def __init__(self, sck: io.PortLike, din: io.PortLike, osc: io.PortLike | None):
        self._sck = sck
        self._din = din
        self._osc = osc
        super().__init__()
    ...
    def elaborate(self, platform):
        ...
        if self._osc is not None:
            osc = Signal()
            m.submodules.osc_buffer = osc_buffer = io.Buffer("o", self._osc)
            m.d.comb += osc_buffer.o.eq(osc)
        ...
        sample_shreg = Signal.like(self.sample_stream.payload)

        m.d.comb += self.sample_stream.payload.eq(sample_shreg)
        with m.If(self.sample_stream.ready):
            m.d.sync += self.sample_stream.valid.eq(0)

        with m.FSM():
            with m.State("IDLE"):
                with m.If(self.control_stream.valid):
                    m.d.comb += self.control_stream.ready.eq(1)
                    m.d.sync += [
                        limit.eq(self.control_stream.payload),
                        count.eq(0),
                    ]
                    m.next = "WAIT"
            ...
            with m.State("SHIFT"):
                ...
                with m.Else():
                    ...
                    with m.If(sck):
                        ...
                        with m.If(count == 24):
                            m.d.sync += self.sample_stream.valid.eq(1)

                        with m.Elif(count >= limit):
                            m.next = "FINISH-XFER"

            with m.State("FINISH-XFER"):
                with m.If(~self.sample_stream.valid):
                    m.next = "IDLE"
...
class SensorHX711Component(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.hx711_controller = hx711_controller = SensorHX711Controller(
            sck=self._ports.sck, din=self._ports.din, osc=self._ports.osc)

        m.d.comb += [
            hx711_controller.sck_divisor.eq(self.sck_divisor),
            hx711_controller.osc_divisor.eq(self.osc_divisor)
        ]

        i_stream_cast = stream.Signature(HX711Setting).create()
        i_stream_cast.valid   = self.i_stream.valid
        i_stream_cast.ready   = self.i_stream.ready
        m.d.comb += i_stream_cast.p.eq(self.i_stream.p[0 : Shape.cast(HX711Setting).width])
        wiring.connect(m, i_stream_cast, hx711_controller.control_stream)

        with m.FSM():
            word_count = Signal(range(3))
            sample = Signal.like(hx711_controller.sample_stream.payload)

            with m.State("IDLE"):
                with m.If(hx711_controller.sample_stream.valid):
                    m.d.comb += hx711_controller.sample_stream.ready.eq(1)
                    m.d.sync += [
                        sample.eq(hx711_controller.sample_stream.payload),
                        word_count.eq(2),
                    ]
                    m.next = "SEND"
...
class SensorHX711Interface:
    def __init__(self, logger: logging.Logger, assembly: AbstractAssembly, *,
        ...
        self._setting = HX711Setting.A_128
    ...
    async def set_channel_gain(self, channel: Literal["A", "B"], gain: Literal[32, 64, 128]):
        ...
        """
        self._setting = HX711Setting.from_channel_gain(channel, gain)
    ...
    async def sample(self) -> int:
        ...
        """
        await self._pipe.send(self._setting.value.to_bytes())
