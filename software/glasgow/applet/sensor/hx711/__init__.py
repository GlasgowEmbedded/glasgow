...
class SensorHX711Interface:
    def __init__(self, logger: logging.Logger, assembly: AbstractAssembly, *,
        ...
        if osc is not None:
            self._osc_interface = ClockDriveInterface(logger, assembly, clk=osc, name="osc")
        else:
            self._osc_interface = None
    ...
    @property
    def osc_interface(self) -> ClockDriveInterface:
        """Interface to optional reference frequency clock.

        The sample rate is either freq/138,240 if the HX711 rate pin is high
        or freq/1,105,920 if it is low.

        Raises
        ------
        HX711Error
            If the optional reference frequency clock is not used
            and :py:`osc_interface` is accessed.
        """
        if self._osc_interface is None:
            raise HX711Error
        else:
            return self._osc_interface
...
class SensorHX711Applet(GlasgowAppletV2):
    ...
    async def setup(self, args):
        ...
        if args.frequency is not None:
            await self.hx711_iface.osc_interface.enable(args.frequency * 1e6)
