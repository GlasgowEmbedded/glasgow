...
class RadioNRF24L01Interface:
    ...
    async def pulse(self):
        await self.enable()
        await asyncio.sleep(0.001) # 10 µs
        await self.disable()
    ...
    async def write_register_wide(self, address: int, value: int):
        assert address in range(0x1f)
    ...
    async def poll_tx_status(self, delay=0.010) -> REG_STATUS:
        ...
        while True:
            async with self._spi_iface.select():
                status_bits, _ = await self._spi_iface.exchange([OP_W_REGISTER|ADDR_STATUS, clear_bits])
            status = REG_STATUS.from_int(status_bits)
            self._log("poll rx status %s", status.bits_repr(omit_zero=True))
            if status_bits & poll_bits:
                break
