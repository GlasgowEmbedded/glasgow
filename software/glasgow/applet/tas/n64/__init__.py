...
class N64TASSubtarget(Elaboratable):
    def __init__(self, pads, out_fifo, in_fifo, micro_cyc, path):
        self.data_t = pads.data_t
    ...
    def send_hex(self, byte):
        self.send_log(Cat(self.hex_char(byte[0:4]), self.hex_char(byte[4:8])), 2)

    def send_char(self, char):
        self.send_byte(self.char_byte(char))
    ...
    def elaborate(self, platform):
        ...
        data_i = Signal(1)
        m.submodules.isync = FFSynchronizer(self.data_t.i, data_i)
        ...
        with m.If(self.in_fifo.w_rdy & (self.log_r != self.log_w)):
            ...
            self.m.d.sync += self.log_r.eq((self.log_r + 1) % self.log_size)
        ...
        m.d.comb += edge_len.eq((3 - 2 * (send_bit != self.data_t.o)) * self.micro_cyc)
        ...
        with m.FSM():
            ...
            with m.State("SEND_LOW"):
                m.d.comb += self.data_t.oe.eq(1)
                m.d.comb += self.data_t.o.eq(0)
                m.d.sync += low_timer.eq(low_timer + 1)
...
class N64TASApplet(GlasgowApplet):
    ...
    def build(self, target, args):
        ...
        subtarget = iface.add_subtarget(N64TASSubtarget(
    ...
    def tests(cls):
        from . import test
        return test.N64TASAppletTestCase
