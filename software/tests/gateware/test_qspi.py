...
from glasgow.gateware.ports import PortGroup, SimulationPort, SimulationPlatform
...
def simulate_flash(ports, memory=b"nya nya nya nya nyaaaaan"):
    ...
    async def watch_cs(cs_o, triggers):
        *values, cs_deasserted = await triggers.posedge(cs_o)
        if cs_deasserted:
            raise CSDeasserted
        return values
        ...
        async def testbench_out(ctx):
            async def bits_get(*, cs, ox, oe, i_en, mode):
                for cycle, o in enumerate(ox):
                    # FIXME: amaranth-lang/amaranth#1414
                    expected = Const({
                        "port": {
