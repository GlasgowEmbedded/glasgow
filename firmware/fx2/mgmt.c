...
/// Actual size of request/response packet (less than or equal to EP1 buffer size of 64 bytes).
/// Pinned to a `__data` global because these variables are accessed a lot, and every saved byte
/// per access multiplies across the entire firmware.
__data uint8_t mgmt_req_len, mgmt_rsp_len;

/// Management request handlers. These are defined within the respective modules, but are not
/// declared in their headers since they can not be called by any other part of the firmware.
...
enum {
  /// Constant meaning "size is determined by the handler".
  MGMT_LEN_VARY = 0xff,
};

typedef __xdata struct {
  /// Which request this handler is for.
  enum mgmt_request type;
  /// Expected length of request payload, or `MGMT_LEN_VARY` to disable length checking so that
  /// it can be implemented in the handler.
  uint8_t req_len;
  /// Constant length of response payoad, or `MGMT_LEN_VARY` to disable length overwrite so that
  /// the value set by the handler prevalis.
  uint8_t rsp_len;
  /// Pointer to the handler function. The calling convention is particularly inefficient for
  /// indirectly called function, so all parameters are passed via globals; see [glasgow_mgmt.h].
...
// Not used for anything except firmware debugging.
enum mgmt_result mgmt_debug()
{
